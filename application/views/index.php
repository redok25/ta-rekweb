			...
			for (let i = 0; i < jum; i++) {
				...
				map.addSource(dataKoor["features"][i]["properties"]['NAMOBJ'] + i, {
					...
					'data': {
						...
						'geometry': {
							'type': 'Polygon',
							'coordinates': dataKoor["features"][i]["geometry"]["coordinates"]
						}
					}
				});
				...
				map.on('click', dataKoor["features"][i]["properties"]['NAMOBJ'] + i, (e) => {
					...
					$.getJSON('<?= base_url() ?>assets/js/data-dummy.json', function(result) {
							...
							buttons: {
								formSubmit: {
									...
									action: function() {
										...
										window.location = "<?= base_url() ?>index.php/tambah-marker?pin=" + cc + "&nama=" + name;
									}
								},
								cancel: function() {
