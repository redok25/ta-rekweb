...
if (isset($_POST['now'])) {
	...
	if ($_POST['now'] == 'login') {
	...
	}elseif ($_POST['now'] == 'ubahPW'){
		$query = $con->query("SELECT * FROM user WHERE username = '".$_SESSION['user']."' ");
		...
		$con->query("UPDATE user SET password = '".password_hash($_POST['pw'], PASSWORD_DEFAULT)."' WHERE username = '".$_SESSION['user']."'");
		header("Location: logic.php?logout=cpw");
